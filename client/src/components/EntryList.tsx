...
import { isChemist, isDoctor } from '../auth'
      ...
      <div className="md:hidden space-y-3">
        {entries.map((entry, index) => {
          const card = (
            <article
              className={`rounded-lg border border-hairline bg-paper p-4 shadow-sm ${
                canViewProfile ? 'hover:border-teal/40 hover:shadow-md transition-shadow' : ''
              }`}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex items-start gap-2.5">
                  ...
                  <div className="min-w-0">
                    ...
                    {canViewProfile ? (
                      <p className="text-[11px] font-semibold text-teal mt-1">Open profile</p>
                    ) : null}
                  </div>
                </div>
          ...
          return canViewProfile ? (
            <Link key={entry.id} to={`/entries/${entry.id}`} className="block">
              {card}
            </Link>
          ) : (
            card
            ...
            {entries.map((entry, i) => (
              <tr
                ...
                className={`border-b border-hairline last:border-0 ${i % 2 === 1 ? 'bg-[#fafcfb]' : 'bg-paper'} ${
                  canViewProfile ? 'cursor-pointer hover:bg-sage/40' : ''
                }`}
                onClick={() => {
                  if (canViewProfile) navigate(`/entries/${entry.id}`)
                }}
              >
